...
* 자식이 2개 이상일 때 부모에 속성을 적용하는 것으로 자식에게 공통 값을 적용할 수 있다.
    ...
    * 꾸미려는 css 속성이 글자에 관련된 속성(글꼴, 글자크기, 글자색상 등) 본인에게 직접 적용하는 것을 권장함.(우선순위가 꼬이기 때문)
## 글꼴 사용 시 주의사항
...
* `<button type="button" 이벤트="자바스크립트명령어작성">`
    * 버튼에 이벤트 작성 시 반드시 type은 button(범용기능) 사용
...
* `position:relative;`
    ...
    * 자식 또는 자손 요소에 absolute가 있어서 부모 기준이 필요할 때
* `position:absolute;`
    * 부모, 형제 요소와 겹치는 디자인 특징이 필요할 때
    8. black 요소에 absolute 설정 시 inline-block 처럼 내용만큼 인식함
...
* 시각적인 목적으로 디자인 배경, 선 등의 요소가 필요할 때 HTML태그없이 CSS만으로도 디자인을 만드는 선택자
* `부모선택자::after {}` -> 부모의 마지막 자식에 디자인 생성
* `부모선택자::before {}` -> 부모의 첫번째 자식에 디자인 생성
### after, before 사용 시 필수 속성
* `content:'';` -> 내용인식 속성, 글자 필요한 디자인이 아닐 경우 '' 빈따옴표
* `display, background-color, width, height`
## 계산기 함수 `calc()`
* +,-,*,/,% 다양한 사칙연산 사용 가능
* 추가 괄호를 통한 복잡한 계산 가능
* **연산자 앞 뒤 여백 필수** `1+1(X)` `1 + 1(O)`
* `width, height, margin, padding` 등 숫자 입력 속성 
### calc() 활용 예시
* `li {width:calc(100% / 4);}`
    * 4개의 li를 같은 크기로 나누기
...
* `flex:flow:row nowrap` : 기본값(메인축 수평, 줄바꿈 X)
* `flex:flow:row wrap` : 메인축 수평, 가로 크기에 따라 줄바꿈 O
* `flex:flow:column nowrap` : 메인축 수직, 줄바꿈 X
* `flex:flow:column wrap` : 메인축 수직, 세로 크기에 따라 줄바꿈 O
...
* `justify-content:`
    ...
    * `space-between` : 메인축이 수직이면 위-아래 양쪽끝, 수평이면 왼쪽-오른쪽 양쪽끝
    * `space-around` : 메인축이 수직이면 위-아래 양쪽여백 주고 균등 배치, 수평이면 왼쪽-오른쪽 양쪽 여백 주고 균등 배치
...
* `:hover` 선택자로 인해 기존속성에서 다른속성으로 변경되는 CSS에 애니메이션을 주고 싶을 때
    ...
    * 2초 기다리고 1초동안 일정속도로 color속성에 애니 적용
## animation + @keyframes
### 적용 순서
...
### 명령 해석
* `@keyframes a {0% {opacity:0;} 100% {opercity:1;}}`
...
### 글자 그라데이션 & 배경이미지 마스크
* `-webkit-` v파이어폭스, 크롬 등 브라우저에서 안전하게 속성 적용하는 웹브라우저 엔진 명령어
...
* 1600~
### 반응형웹 설정
1. 설계방향 mobile -> PC 점점 크게(min-width)
2. 설계방향 PC -> mobile 점점 작게(max-width)
* `@media screen and (규칙) {실행명령어}`
* `@media screen and (w320~) {320이상일때 재배열 명령어}`
* `@media screen and (w720~) {720이상일때 재배열 명령어}`
* media 규칙괄호 안에는 세미콜론(;) 금지
### 반응형 제작 주의사항
1. @media 규칙선언전 기존CSS에서 유지하고 싶은건 쓰지않기
2. @media 규칙선언 후 재배열하고 싶은 CSS는 반드시 기존 CSS에서 어느 선택자에 어떤값으로 썼는지 확인하고 동일한 선택자에 동일속성으로 값만 바꿔서 입력하기
## input 디자인
* `<input type="radio">`
* `<input type="checkbox">`
* `<option><select>`
...
1. radio, chechbox와 형제관계 태그 준비 (예)label, span 등
